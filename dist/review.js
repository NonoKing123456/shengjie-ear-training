...
function renderReview(correct){
  ...
  $('#sound-stage').classList.add('is-review');$('#stage-prompt').classList.add('is-hidden');panel.classList.remove('is-hidden');
...
$('#review-content').addEventListener('click',event=>{
  const target=event.target.closest('[data-next],[data-play],.score-note');if(!target)return;
